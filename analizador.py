...
def detect_objects(video_path, video_title, show_video, frame_step):
    # Limpiar el título del video para el nombre de la carpeta
    base_title = clean_filename(video_title)
    
    # Crear estructura de carpetas igual que en process_video
    base_folder = "Fotogramas"
    current_title = base_title
    counter = 1
    
    while os.path.exists(os.path.join(base_folder, current_title)):
        current_title = f"{base_title}_{counter}"
        counter += 1
    ...
    while cap.isOpened():
        ...
        if len(indexes) > 0:
            ...
            frame_filename = os.path.join(output_folder, f'object_{formatted_time.replace(":", "-")}.jpg')
            cv2.imwrite(frame_filename, frame)
    ...
    csv_filename = None
    if results:
        csv_filename = os.path.join(csv_folder, f'{current_title}_objects.csv')
        with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['time', 'title', 'description', 'object', 'confidence']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(results)
